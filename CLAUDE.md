...
- **Headless harness** at [plugins/note_detect/tools/harness.js](plugins/note_detect/tools/harness.js) runs the same `processFrame` / `matchNotes` / `checkMisses` code path off Node, in seconds per run. Same `note_detect.diagnostic.v1` schema as the in-app Download Diagnostic button.
